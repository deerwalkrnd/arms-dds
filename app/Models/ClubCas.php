...
class ClubCas extends Model
{
    ...
    public function clubAssignment(): BelongsTo
    {
        return $this->belongsTo(ClubAssignment::class, "clubAssignment_id");
    }
    ...
    public static function calculateTotalMarksPerSubjectTeacher($subjectId, $studentId, $startDate, $endDate)
    {
        ...
        // put filter here
        $casRecords = ClubCas::whereHas("clubAssignment.subjectTeacher.subject", function ($query) use ($subjectId) {
            $query->where("subject_id", $subjectId);
        })
            ->where('student_id', $studentId)
            ->whereHas("clubAssignment", function ($query) use ($term) {
                return $query->where("term_id", $term->id);
            })
            ->get()->sortBy("clubAssignment.eca_cas_type_id");
        // -----------------------------------------------------
        ...
        foreach ($casRecords as $cas) {
            // put filter here
            $casTypeId = $cas->clubAssignment->ecaCasType->id;
            // ----------------------------------------------
            ...
            $casTypeStats[$casTypeId]['count'] = isset($casTypeStats[$casTypeId]['count']) ?
                ...
                1;
            $casTypeStats[$casTypeId]['name'] = $cas->clubAssignment->ecaCasType->name;
        }
        ...
        foreach ($casTypeStats as $casTypeId => $stats) {
            ...
            $fullMarksPerCasType[$casTypeId] = EcaCasType::findOrFail($casTypeId)->full_marks;
