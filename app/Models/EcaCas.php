...
class EcaCas extends Model
{
    ...
    public function ecaAssignment(): BelongsTo
    {
        return $this->belongsTo(EcaAssignment::class, "ecaAssignment_id");
    }
    public static function calculateTotalMarksPerSubjectTeacher($subjectId, $studentId, $startDate, $endDate)
    {
        ...
        // put filter here
        $casRecords = EcaCas::whereHas("ecaAssignment.subjectTeacher.subject", function ($query) use ($subjectId) {
            $query->where("subject_id", $subjectId);
        })
            ->where('student_id', $studentId)
            ->whereHas("ecaAssignment", function ($query) use ($term) {
                return $query->where("term_id", $term->id);
            })
            ->get()->sortBy("ecaAssignment.eca_cas_type_id");
        // -----------------------------------------------------
        ...
        foreach ($casRecords as $cas) {
            // put filter here
            $casTypeId = $cas->ecaAssignment->ecaCasType->id;
            // ----------------------------------------------
            ...
            $casTypeStats[$casTypeId]['count'] = isset($casTypeStats[$casTypeId]['count']) ?
                ...
                1;
            $casTypeStats[$casTypeId]['name'] = $cas->ecaAssignment->ecaCasType->name;
        }
