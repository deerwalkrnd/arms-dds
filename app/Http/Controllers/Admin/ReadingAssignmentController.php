...
class ReadingAssignmentController extends Controller
{
    public function edit(int $assignmentId)
    {
        try {
            ...
            $assignment = ReadingAssignment::where('id', $assignmentId)->firstOrFail();


            $subjectTeacher = $assignment->subjectTeacher;

            // Retrieve the castypes from the given grade request
            $casTypes = ReadingCasType::where("school_id", $subjectTeacher->section->grade->school_id)->get();
            ...
            return view("admin.readingAssignments.edit", compact('assignment', 'casTypes', 'cas'));
        } catch (Exception $e) {
    ...
    public function updateAndSave(int $assignmentId, ReadingTeacherCasStoreRequest $request)
    {
        ...
        try {
            ...
            $schoolId=$assignment->readingCasType->school->id;
