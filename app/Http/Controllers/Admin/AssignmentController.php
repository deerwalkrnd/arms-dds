...
class AssignmentController extends Controller
{
    ...
    public function options(){
        $schools=School::all()->sortDesc();
        return view('admin.assignments.selectSchool', compact('schools'));
    }
    public function select(Request $request){
      try{  $data=$request->validate([
            'school' => 'required',
        ]);
        return redirect()->route('adminAssignments.index', ['id' => $data['school']]);
    }catch(Exception $e){
    ...
    {
        $schools=School::all()->sortDesc();
        $teacher_id = School::where('id',$id)->firstOrFail()->headOfSchool->id;
        // dd($teacher_id);
        $school=School::where('id',$id)->firstOrFail()->name;
        ...
        $sortedAssignments = $allAssignments->sortByDesc('created_at');

        return view("admin.assignments.index", compact("sortedAssignments","schools",'school'));
    }
    ...
    public function view(int $assignmentId)
    {
        try {
            ...
            return view("admin.assignments.view", compact("assignment", "cas"));
        } catch (Exception $e) {
    ...
    public function edit(int $assignmentId)
    {
        try {
            ...
            $casTypes = CasType::where("school_id", $subjectTeacher->section->grade->school_id)->get();
            ...
            return view("admin.assignments.edit", compact('assignment', 'casTypes', 'cas'));
        } catch (Exception $e) {
            Log::error($e->getMessage());
            return redirect()->back()->withErrors(["Error", "Assignment not found"]);
        }
    ...
    public function updateAndSave(int $assignmentId, TeacherCasStoreRequest $request)
    {
        ...
        try {
            ...
            $schoolId=$assignment->casType->school->id;
    ...
    public function reportIndex()
    {
        try {
            ...
            return view("admin.assignments.reportIndex", compact("terms", "subjectTeachers", "casLatest", "latestSubjectTeacher", "cas", "students", "casTypes"));
    ...
    public function reportSearch(Request $request)
    {
        try {
            ...
            if ($isNullCasType && ($isNullSubjectTeacher && $isNullTerm)) {
                return redirect(route("adminCasReport.index"))->withErrors(["Error", "Please select at least first two filters"]);
            }
            ...
            if (!$termExists) {
                return redirect(route("adminCasReport.index"))->withErrors(["Error", "Term Doesnot Belong to Subject"]);
            }
            ...
            if ($subjectType == "MAIN" || $subjectType == "CREDIT") {
                ...
                $casTypes = ReadingCasType::all()->sortBy("school.name");
            } else {
                $casTypes = EcaCasType::all()->sortBy("school.name");
            }
            ...
            if ($isNullCasType) {
                ...
                if ($isNullTerm || $isNullSubjectTeacher) {
                    return redirect(route("adminCasReport.index"))->withErrors(["Error", "Term or Subject not selected"]);
                }
                ...
                if ($cas->count() == 0) {
                    return redirect(route("adminCasReport.index"))->withErrors(["Error", "No CAS Marks present"]);
                }
            } else {
            ...
            return view("admin.assignments.reportSearch", compact("terms", "subjectTeachers", "cas", "students", "casTypes", "subjectTeacher", "term"));

        } catch (Exception $e) {
            Log::error($e->getMessage());
            return redirect(route("adminCasReport.index"))->withErrors(["Error", "Search Filter not valid: "]);
        }
    }
