...
class CasController extends Controller
{
    ...
    public function index()
    {
        ...
        return view("admin.cas.index")->with(compact("cas"));
    }
    ...
    public function create(int $subjectTeacherId)
    {
        try {
            ...
            $subjectTeacher = SubjectTeacher::where("id", $subjectTeacherId)->where("teacher_id", $teacher_id)->firstOrFail();
            ...
            $students = Student::whereHas("section.subjectTeachers", function ($query) use ($subjectTeacher) {
     ...
     */
    public function store(CasStoreRequest $request, $id)
    {
        ...
        try {
            ...
            DB::rollBack();
            return back()->withInput()->withErrors(["errors" => "Failed to create cas marks"]);
        }
    }
    ...
    public function edit($id)
    {
        ...
        $students = Student::whereHas("section.subjectTeachers", function ($query) use ($teacher_id) {
            $query->where("teacher_id", $teacher_id);
        })->get()->sortBy('name');
    ...
    public function update(CasRequest $request, string $id)
    {
        try {
            ...
            return redirect()->back()->withInput()->withErrors(["errors" => "Failed to edit cas marks"]);
        }
    }
    ...
    public function destroy(string $id)
    {
        try {
            ...
            return redirect()->back()->withErrors(["errors" => "Failed to delete cas marks"]);
        }
    }
    ...
    public function newReportIndex(int $subjectTeacher)
    {
        $subjectTeacher = SubjectTeacher::where("id", $subjectTeacher)->where("teacher_id", auth()->id())->firstOrFail();
        $students = $subjectTeacher->section->students;
        $casTypes = $subjectTeacher->section->grade->school->casTypes;
        $assignments = $subjectTeacher->assignments->groupBy("casType.name");
    }

    public function newReportSearch(int $subjectTeacher, Request $request)
    {
