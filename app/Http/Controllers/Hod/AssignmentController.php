...
class AssignmentController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $ecaAssignments = EcaAssignment::whereHas("subjectTeacher.subject.department", function ($query) use ($teacherId) {
                $query->where("head_of_department_id", $teacherId);
            })->with(['subjectTeacher', 'ecaCasType'])->get();

            $clubAssignments = ClubAssignment::whereHas("subjectTeacher.subject.department", function ($query) use ($teacherId) {
                $query->where("head_of_department_id", $teacherId);
            })->with(['subjectTeacher', 'ecaCasType'])->get();
            $readingAssignments = ReadingAssignment::whereHas("subjectTeacher.subject.department", function ($query) use ($teacherId) {
                $query->where("head_of_department_id", $teacherId);
            ...
            $sortedAssignments = $allAssignments->sortByDesc('date_assigned');
            return view("hod.assignment.index", compact("assignments","ecaAssignments", "clubAssignments","readingAssignments","sortedAssignments"));
        } catch (Exception $e) {
