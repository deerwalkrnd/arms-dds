...
class ExamController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $terms = Term::whereHas("grade.sections.subjectTeachers.subject.department", function ($query) use ($teacherId) {
                return $query->where("head_of_department_id", $teacherId);
            })->get()->sortBy('grade.name');
    ...
    public function view(int $termId, int $subjectId)
    {
        try {
            ...
            $term = Term::with('exams')->where("id", $termId)->whereHas("grade.sections.subjectTeachers.subject.department", function ($query) use ($teacherId) {
                return $query->where("head_of_department_id", $teacherId);
            })->first();
