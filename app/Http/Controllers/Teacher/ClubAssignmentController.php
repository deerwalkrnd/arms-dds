...
class ClubAssignmentController extends Controller
{
    public function view(int $assignmentId)
    {
        try {
            ...
            $assignment = ClubAssignment::where("id", $assignmentId)->whereHas("subjectTeacher.teacher", function ($query) use ($teacherId) {
            ...
            return view("teacher.clubAssignment.view", compact("assignment", "cas"));
    ...
    public function edit(int $id)
    {
        try {
            ...
            $assignment = ClubAssignment::where('id', $id)->whereHas('subjectTeacher.teacher', function ($query) use ($teacherId) {
                return $query->where("id", $teacherId);
            })->firstOrFail();
            ...
            return view("teacher.clubAssignment.edit", compact('assignment', 'casTypes', 'cas'));
    ...
    public function updateAndSave(int $assignmentId, ClubTeacherCasStoreRequest $request)
    {
        ...
        try {
            ...
            foreach ($data['students'] as $id => $studentId) {
                $cas = ClubCas::where("student_id", $studentId)->where("clubAssignment_id", $assignment->id)->whereHas('student',function ($query){
                    return $query->where("status","ACTIVE");
                })->firstOrFail();
