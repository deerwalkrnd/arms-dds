...
class SubjectController extends Controller
{
    ...
    public function create()
    {
        try {
            ...
            return view('hos.subjects.create', compact("departments", "grades","types"));
        } catch (Exception $e) {
            Log::error($e->getMessage());
    ...
    public function destroy(int $subjectId)
    {
        try {
            $subject = Subject::findOrFail($subjectId);
            $subjectTeacher= SubjectTeacher::where("subject_id", $subject->id)->exists();
            if($subjectTeacher){
                return redirect()->back()->withErrors(["errors" => "Subject Teacher exists for the subject."]);
            }
            $subject->delete();
            ...
            Log::error($e->getMessage());
            return redirect()->back()->withInput()->withErrors(['Error', "Failed to delete subject"]);
        }
    }
}
