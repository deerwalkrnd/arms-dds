...
class EcaAssignmentController extends Controller
{
    public function edit(int $assignmentId)
    {
        try {
            ...
            $assignment = EcaAssignment::where('id', $assignmentId)->whereHas('subjectTeacher.subject.grade.school', function ($query) use ($teacherId) {
            ...
            return view("hos.ecaAssignments.edit", compact('assignment', 'casTypes', 'cas'));
        } catch (Exception $e) {
    ...
    public function updateAndSave(int $assignmentId, EcaTeacherCasStoreRequest $request)
    {
        ...
        try {
            ...
            foreach ($data['students'] as $id => $studentId) {
                ...
                $cas = EcaCas::where("student_id", $studentId)->whereHas('student',function ($query){
                    return $query->where("status","ACTIVE");
                })->where("ecaAssignment_id", $assignment->id)->firstOrFail();
