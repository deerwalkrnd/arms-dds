...
@section('title')
    ...
    <div>CAS Report of - <span class="text-custom-black font-normal">{{ $subjectTeacher->subject->name }}
...
@section('content')
    <div class="w-full flex flex-col gap-2">
        ...
        <div class="pt-3 pb-2 px-2">
            <form method="POST" action="{{ route('adminCasReport.search') }}">
                @csrf
                @method('POST')
                ...
                <div class="text-base font-normal flex gap-2">
                    ...
                    <select name="subjectTeacher" id="subjectTeacher"
                        ...
                        {{-- Place holder option --}}
                        <option value="{{ null }}" class="bg-white text-black selectOptionSubject" id="placeholderSubject">--Select
                            Subject--
                        </option> 
                    ...
                    <select name="term" id="term"
                        ...
                        <option value="{{ null }}"  class="bg-white text-black selectOptionTerm" id="placeholderTerm">--Select a subject --
                        </option>
        ...
        $(document).ready(function() {
            ...
            $('#subjectTeacher').change(function() {
                ...
                if(selectedSubjectTeacherId){
                    $.ajax({
                        type: "POST",
                        url: '{{ route('adminCasReport.populateTerm') }}',
                        data: {
                            "_token": "{{ csrf_token() }}",
                    ...
                    $.ajax({
                        type: "POST",
                        url: '{{ route('adminCasReport.populateCasType') }}',
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "subjectTeacherId": selectedSubjectTeacherId
                        },
