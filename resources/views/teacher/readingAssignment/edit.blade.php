                ...
                <div class="flex flex-col">
                    ...
                    <select name="cas_type" id="cas_type" class="border border-custom-black rounded-sm px-3 py-1 grow">
                        ...
                        @foreach ($casTypes as $casType)
                            @if (old('cas_type') == $casType->id || $assignment->reading_cas_type_id == $casType->id)
...
@section('script')
    <script>
        $("#saveForm").click(function() {
            $("#form").attr("action", "{{ route('teacherReadingAssignments.updateAndSave', (int) $assignment->id) }}")
        })
        ...
        $("#saveAndSubmitForm").click(function(e) {
            e.preventDefault();
            $("#form").attr("action", "{{ route('teacherReadingAssignments.updateAndStore', (int) $assignment->id) }}");
