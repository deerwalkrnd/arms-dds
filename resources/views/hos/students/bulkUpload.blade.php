...
<div class="w-full">
    <div>
        <a href="{{ route('hosStudents.bulkSample') }}">
            <button
    ...
    <form action="{{ route('hosStudents.bulkUpload') }}" method="POST" name="myform" enctype="multipart/form-data">
        @csrf
        <div class="flex flex-col gap-4  mt-8 justify-center items-center">
            ...
            <div class="flex justify-end pb-6 float-end mt-4  ">
                <button type="submit"
                    class="bg-dark-orange shadow-2xl text-white font-semibold text-lg px-4 py-3  rounded-md">Bulk Upload
                    Student</button>
            </div>
        </div>
    </form>
