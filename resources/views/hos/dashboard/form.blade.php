...
<select name="markType" class="bg-dark-orange text-white px-3 py-3 rounded-md markType text-base font-normal">
...
@section('content')
<div class="py-3 w-full min-h-fit border">
        ...
        <div class="mt-5">
            ...
            <table class="table-auto w-full hidden" id="examTable">
                ...
                <tbody>
                    ...
                    <tr class="h-16 border-b-2 hover:bg-dark-gray">
                        <td class="text-center">{{ $studentExam->roll_number }}</td>
                        <td class="px-2 text-left">{{ $studentExam->name }}</td>
                        <td class="px-2 text-left">{{ $studentExam->section->name }}</td>
                        <td class="px-2 text-left">
...
<script>
    $('.markType').change(
        ...
        $("#saveForm").click(function() {
            $("#form").attr("action", "{{ route('hosForms.saveCas', (int) $subjectTeacher->id) }}");
            $("#form").submit();
        })
        ...
        $("#saveAndSubmitForm").click(function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $("#form").attr("action",
                        "{{  route('hosForms.storeCas', (int) $subjectTeacher->id) }}");
                    $("#form").submit();
                }
            });
        })
        ...
        $("#saveExamForm").click(function() {
            $("#form").attr("action", "{{ route('hosForms.storeExam', (int) $subjectTeacher->id) }}");
            $('#saveExamForm').disabled = true;
            // $("#form").submit();
        })
</script>
@endsection